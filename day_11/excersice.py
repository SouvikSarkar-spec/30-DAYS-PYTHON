...
def is_empty(a):
    if a==[]:
      return f"{a} is empty"
       ...
       return f"{a} is not empty"

print(is_empty([1,23,4]))
...
def calculate_mean(a):
    ...
    for num in a:
        total+=num
    return total/len(a)
print(calculate_mean([1,4,5,6,7,8,9]))
...
#     laist.sort(reverse=True)
#     y=laist[0]
#     return f"the mode is {y[1]}"
 
# print(calculate_mode([1,4,6,5,6,7,8,9]))
...
def calculate_median(a):
    ...
    if len(a)%2==0:
        mid1=a[len(a)//2]
        mid2=a[(len(a)-1)//2]
        return f"the median of {a} is {(mid1+mid2)/2}"
    else:
        return f"the median of {a} is {a[len(a)//2]}"
print(calculate_median([1,4,6,5,6,7,8,9]))
...
def calculate_range(a):
    ...
    big=a[-1]
    range_val=big-small
    return f"range is {range_val}"
print(calculate_range([1,4,6,5,6,7,8,9]))
...
print(cal_variance([1,4,6,5,6,7,8,9]))
def cal_std(a):
    return f"the std is {cal_variance(a)**0.5}" 
print(cal_std([1,4,6,5,6,7,8,9]))
...
def all_unique(lst):
    for item in lst:
       if lst.count(item) > 1:
        return False  
    return True
print(all_unique([1,5,4,2,6,4,8,5,1,5,8,2]))

def same_data_type(a):
   x=type(a[0])
   for y in a:
      if type(y)!=x:
         return f"not same type"
    
   return f"same type" 
print(same_data_type([1,5,4,True,6,4,8,5,1,5,8,2]))
def valid_python(x):
    valid_types = (bool, int, float, str, list)

    if type(x) in valid_types:
        return "valid datatype"

    return "invalid datatype"
print(valid_python(1))
...
def most_spoken_languages(limit):
    a=[a for country in data for a in country['languages']]
    b=[(a.count(lang),lang) for lang in set(a)]
    b.sort(reverse=True)
    b=b[:limit]
    return b
...
def most_populated_country(limit):
    b=[(b['population'],b['name']) for b in data]
    b.sort(reverse=True)
